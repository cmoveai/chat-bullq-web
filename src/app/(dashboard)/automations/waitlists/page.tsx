...
export default function WaitlistsPage() {
  ...
  const handleCreate = () => {
    toast('Em breve · feature em construção', {
      description:
        'A criação de listas de espera estará disponível na próxima atualização',
    });
  };
  ...
  return (
    <div className="flex h-full flex-col gap-6 p-6">
      <PageHeader
        ...
        description="Crie e gerencie suas páginas de captura de lista de espera"
        actions={
          <button
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total de Páginas" value={0} icon={<Clock className="h-4 w-4" />} />
        <StatCard label="Ativo" value={0} icon={<CheckCircle className="h-4 w-4" />} />
        <StatCard label="Total de Inscrições" value={0} icon={<Users className="h-4 w-4" />} />
        <StatCard label="Hoje" value={0} icon={<Calendar className="h-4 w-4" />} />
      </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-lg border border-zinc-200 bg-white py-16 text-center dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div>
          <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
            Nenhuma lista de espera ainda
          </p>
          <p className="mt-1 text-xs text-zinc-500">
            Crie sua primeira lista de espera para começar a coletar inscrições.
          </p>
        </div>
        <button
          onClick={handleCreate}
        ...
        >
          <Plus className="h-4 w-4" />
          Crie sua primeira lista de espera
        </button>
      </div>
    </div>
  );
