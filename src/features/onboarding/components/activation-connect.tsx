...
import { Plug, Bot, MessageCircle, ArrowLeft, LifeBuoy, LogOut, Check } from 'lucide-react';
import { MetaIcon, InstagramIcon } from '@/components/ui/icons';
import { useAuthStore } from '@/stores/auth-store';
import {
  ChannelConnectModal,
  type ConnectVariant,
} from '@/features/channels/components/channel-connect-modal';
...
function typeForVariant(v: ConnectVariant): ChannelType {
  return v === 'whatsapp' ? 'WHATSAPP_OFFICIAL' : 'INSTAGRAM';
}

/** Experiência de ativação (1º canal). overlay = gate bloqueante; page = conteúdo
 *  da área de canais em Activation Mode. Em ambos: cancelar volta pra cá, e
 *  nada conecta sozinho. */
export function ActivationConnect({ variant }: { variant: 'overlay' | 'page' }) {
  ...
  // "Continuar" do modal educativo → abre o fluxo do tipo certo (sem tela técnica).
  const goToFlow = () => {
    if (modal) setCreateType(typeForVariant(modal));
    setModal(null);
    setIgChoice(false);
  };
  ...
  const panel = (
    <div
      className={
        variant === 'overlay'
          ? 'relative w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-[0_24px_70px_-20px_rgba(0,0,0,0.35)] ring-1 ring-zinc-900/5 dark:bg-zinc-900 dark:ring-white/10'
          : 'mx-auto w-full max-w-xl overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900'
      }
    >
      <div className="px-7 pt-8 sm:px-9">
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
          Escolha WhatsApp ou Instagram para ativar sua operação na EIXXO e começar a centralizar
          conversas, automatizar respostas e qualificar leads com IA.
        </p>
        ...
        <div className="mt-6 flex items-center gap-3">
          {STEPS.map((s, i) => (
            <div key={s.label} className="flex items-center gap-3">
              ...
              {i < STEPS.length - 1 && <span className="h-px w-5 bg-zinc-200 dark:bg-zinc-700" />}
            </div>
          ))}
        </div>
      </div>
